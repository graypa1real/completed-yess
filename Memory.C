...
uint64_t Memory::getLong(int32_t address, bool & imem_error)
{
   uint64_t ret = 0;
   imem_error = false;
   if((address % 8) != 0 || (address + 8) > MEMSIZE || address < 0)
   {
       imem_error = true;
       return 0;
   }
   ...
   for(int i = 0; i < 8; i++)
   {
       debug =  (mem[address]);  //adds the byte bit shifted to the right place
       shift = 8 * i;  // shift and debug are there because I needed to find the problem, and now that it works I am to lazy to remove them
       //everything here is 1 step at the time.
       debug = debug << shift;
       ret = ret | debug;
       address++;
   }
...
{
   if(address >= 0 && (address + 8) <= MEMSIZE && (address % 8) == 0)
   {
       int a;
       
       for(int i = 0; i < 8; i++)
       {
           a = Tools::getByte(value, i);
           mem[address] = a;
           address++;
       }
