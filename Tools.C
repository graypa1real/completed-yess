...
{
   int indexOfByte = 7;
   long current = bytes[indexOfByte];
   indexOfByte = indexOfByte - 1;
   //loop shifts the value over the distance of a bit and then adds the next bit in
   //then it decrements the index
   //this should run until the index has finished the cycle with an index of 0
   while (indexOfByte >= 0)
   {   
       current = (current << 8) | bytes[indexOfByte];
       indexOfByte = indexOfByte - 1;
   }
   return current;
}
...
uint64_t Tools::getByte(uint64_t source, int32_t byteNum)
{
   ...
   {
    long sourceShifted = 0;
    sourceShifted = source >> (byteNum * 8);
    neededByte = sourceShifted & 255; 
   }
   return neededByte;
...
uint64_t Tools::getBits(uint64_t source, int32_t low, int32_t high)
{
   ...
   if (low >= 0 && low < 64 && high < 64 && high >= 0 && high >= low)
   {
...
uint64_t Tools::setBits(uint64_t source, int32_t low, int32_t high)
{
   ...
   if (low >= 0 && low < 64 && high >= 0 && high < 64 && high >= low)
   {
       mask = mask << (63 - high);
...
{
   uint64_t mask = 0xffffffffffffffff;
   if ( low >= 0 && low < 64 && high >= 0 && high < 64 && high >= 0)
   {
       mask = mask << (63 - high);
       mask = mask >> (63 - high + low);
       mask = mask << low;
                         ...
                         int32_t srclow, int32_t dstlow, int32_t length)
{
   ...
   if(srclow >=0 && srclow < 64 && dstlow >=0 && dstlow < 64 && (dstlow + length) <= 64 && (srclow + length) <= 64 && length > 0)
   {
      
      destination = clearBits(dest, dstlow, (dstlow + length - 1));
      //std::cout<<std::hex<<destination<<"\n";
      source = source << (64 - (srclow + length));
...
uint64_t Tools::setByte(uint64_t source, int32_t byteNum)
{
   ...
   if(byteNum < 8 && byteNum >=0)
   {
       ...
       answer = setBits(source, (byteNum * 8), (byteNum * 8 + 7));
   }
   return answer;
}
...
{
   int over = 0;
   if( op1 >>63 && op2 >> 63 && !((op1+op2) >> 63))
   {
       over = 1;
   }
   else if(!(op1 >> 63) && !(op2 >> 63) && (op1 + op2) >> 63)
       {
           over = 1;
       }
   return over;
...
{
   return (((op1>>63) != (op2>>63)) && (((op1-op2)>>63) != (op1>>63)));
}
